...
app.engine("hbs", exphbs.engine({
  extname: ".hbs",
  helpers: {
    eq: (a, b) => a === b // helper for select options in edit forms
  }
}));
...
app.use(session({
  secret: "supersecret",   // replace with something unique in production
  resave: false,
...
app.post("/add/weapon", (req, res) => {
  const { name, description, image_url, class: weaponClass, damage } = req.body;
  db.run(
    "INSERT INTO weapons (name, class, description, image_url, damage) VALUES (?, ?, ?, ?, ?)",
